...
int I2CIO::begin (  uint8_t i2cAddr )
{
   ...
   // Configure the TDI port expander
   Wire.begin(5,4); // SClockPin and DataPin
   ...
   Serial.println("init of port expander done ...\n");
...
#if (ARDUINO <  100)
      _shadow = (Wire.receive ()^_dirMask);
#else
      _shadow = ( Wire.read ( ) ^ _dirMask );
#endif

      Wire.beginTransmission ( _i2cAddr );
#if (ARDUINO <  100)
      Wire.send ( _shadow | _dirMask );
#else
      Wire.write ( _shadow | _dirMask );
#endif 
